class ProjectsController < ApplicationController
  before_action :set_project, only: %i[index show update destroy edit ]
  ...
  def update
    ...
    if removed_ids.present?
      blocked_ids = Bug.where(project_id: @project.id)
                      .where("assignee_qa_id IN (?) OR assignee_dev_id IN (?)", removed_ids, removed_ids)
                      .pluck(:assignee_qa_id, :assignee_dev_id)
                      .flatten.uniq & removed_ids

      if blocked_ids.present?
        blocked_names = User.where(id: blocked_ids).pluck(:name).join(", ")
        return redirect_to edit_project_path(@project), alert: "You can't remove user(s): #{blocked_names} with ongoing bugs."
      end
    end
